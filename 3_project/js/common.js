$(document).ready(function (){
	...
	if (dep1 >= 0) {
		...
		var dep2Clone = $depth1.next().clone(); //#snb .dep2 ul을 복제해 둠
	...
	//모바일 닫기에서 shift+tab을 눌러 나가려는 경우
	$(document).on('keydown', '.close', function (e) {
		if (e.shiftKey && e.keyCode == 9) {
			e.preventDefault();   //★ 기본 기능을 먼저 제한하고 포커스 강제 이동시키기
			$gnb.find('[data-link="last"]').focus();
		}
	});

	//열려진 모달 gnb의 가장 마지막 a 태그에서 tab을 눌러 나가려는 경우
